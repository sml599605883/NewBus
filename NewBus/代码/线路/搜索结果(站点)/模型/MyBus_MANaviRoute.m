...
#import "MyBus_CommonUtility.h"

#define kMANaviRouteReplenishPolylineFilter     5

@interface MyBus_MANaviRoute()
...
@implementation MyBus_MANaviRoute
...
+ (MyBus_MANaviRoute *)naviRouteForRailway:(AMapRailway *)railway
{
    ...
    for (int i = 0; i < stations.count - 1; i++)
    {
        ...
        MyBus_MANaviPolyline *naviPolyline = [[MyBus_MANaviPolyline alloc] initWithPolyline:polyline];
        naviPolyline.type = MANaviAnnotationTypeRailway;
        
        [polylines addObject:naviPolyline];
        
        MyBus_MANaviAnnotation * annotation = [[MyBus_MANaviAnnotation alloc] init];
        ...
        {
            // add last station
            MyBus_MANaviAnnotation *lannotation = [[MyBus_MANaviAnnotation alloc] init];
    ...
    return [MyBus_MANaviRoute naviRouteForPolylines:polylines andAnnotations:naviAnnotations];
}

+ (MyBus_MANaviRoute *)naviRouteForTaxi:(AMapTaxi *)taxi
{
    if (taxi == nil)
    {
        return nil;
...
+ (MyBus_MANaviRoute *)naviRouteForWalking:(AMapWalking *)walking
{
    ...
    [walking.steps enumerateObjectsUsingBlock:^(AMapStep *step, NSUInteger idx, BOOL *stop) {
        ...
        if (stepPolyline != nil)
        {
            MyBus_MANaviPolyline *naviPolyline = [[MyBus_MANaviPolyline alloc] initWithPolyline:stepPolyline];
...
+ (MyBus_MANaviRoute *)naviRouteForSegment:(AMapSegment *)segment
{
    if (segment == nil)
    {
        return nil;
    ...
    MyBus_MANaviRoute *walkingRoute = [MyBus_MANaviRoute naviRouteForWalking:segment.walking];
    if (walkingRoute)
    {
    ...
    // taxi
    MyBus_MANaviRoute *taxiRoute = [MyBus_MANaviRoute naviRouteForTaxi:segment.taxi];
    if (taxiRoute)
    {
    ...
    // railway
    MyBus_MANaviRoute *railwayRoute = [MyBus_MANaviRoute naviRouteForRailway:segment.railway];
    if (railwayRoute)
    {
    ...
    MAPolyline *busLinePolyline = [MyBus_MANaviRoute polylineForBusLine:firstLine];
    if (busLinePolyline)
    {
        MyBus_MANaviPolyline *naviPolyline = [[MyBus_MANaviPolyline alloc] initWithPolyline:busLinePolyline];
        naviPolyline.type = MANaviAnnotationTypeBus;
        
        [polylines addObject:naviPolyline];
        
        MyBus_MANaviAnnotation * bus = [[MyBus_MANaviAnnotation alloc] init];
    ...
    return [MyBus_MANaviRoute naviRouteForPolylines:polylines andAnnotations:annotations];
...
+ (MAPolyline *)polylineForStep:(AMapStep *)step
{
    ...
    return [MyBus_CommonUtility polylineForCoordinateString:step.polyline];
}
...
+ (MAPolyline *)polylineForBusLine:(AMapBusLine *)busLine
{
    ...
    return [MyBus_CommonUtility polylineForCoordinateString:busLine.polyline];
}
                              ...
                              Polylines:(NSMutableArray *)polylines
{
    ...
    MyBus_LineDashPolyline *startDashPolyline = nil;
    MyBus_LineDashPolyline *endDashPolyline = nil;
    if (start)
    {
        ...
        MyBus_MANaviPolyline *naviPolyline = [polylines firstObject];
        MAPolyline *polyline = nil;
        if ([naviPolyline isKindOfClass:[MyBus_MANaviPolyline class]])
        {
    ...
    if (end)
    {
        ...
        MyBus_MANaviPolyline *naviPolyline = [polylines lastObject];
        MAPolyline *polyline = nil;
        if ([naviPolyline isKindOfClass:[MyBus_MANaviPolyline class]])
        {
            polyline = naviPolyline.polyline;
                                 ...
                                 Walking:(AMapWalking *)walking
{
    ...
    {
        MyBus_LineDashPolyline *dashPolyline = [self replenishPolylineWithStart:startCoor end:endCoor];
        if (dashPolyline)
        {
...
+ (MyBus_LineDashPolyline *)replenishPolylineWithStart:(CLLocationCoordinate2D)startCoor end:(CLLocationCoordinate2D)endCoor
{
    ...
    MyBus_LineDashPolyline *dashPolyline = nil;
    ...
    if (distance > kMANaviRouteReplenishPolylineFilter)
    {
        ...
        dashPolyline = [[MyBus_LineDashPolyline alloc] initWithPolyline:polyline];
    }
...
- (instancetype)initWithTransit:(AMapTransit *)transit startPoint:(AMapGeoPoint *)start endPoint:(AMapGeoPoint *)end
{
    ...
    [transit.segments enumerateObjectsUsingBlock:^(AMapSegment *segment, NSUInteger idx, BOOL *stop) {
        ...
        MyBus_MANaviRoute * routeSegment = [MyBus_MANaviRoute naviRouteForSegment:segment];
        ...
        if (idx > 0)
        {
            [MyBus_MANaviRoute replenishPolylinesForTransit:[transit.segments objectAtIndex:idx-1] CurrentSegment:segment Polylines:polylines];
        }
    }];
    
    [MyBus_MANaviRoute replenishPolylinesForStartPoint:start endPoint:end Polylines:polylines];
...
- (instancetype)initWithPath:(AMapPath *)path withNaviType:(MANaviAnnotationType)type showTraffic:(BOOL)showTraffic startPoint:(AMapGeoPoint *)start endPoint:(AMapGeoPoint *)end
{
    ...
    if (showTraffic && type == MANaviAnnotationTypeDrive)
    {
        ...
        MAPolyline *polyline = [MyBus_MANaviRoute multiColoredPolylineWithDrivePath:path polylineColors:&polylineColors];
        if (polyline)
        {
    ...
    else
    {
        [path.steps enumerateObjectsUsingBlock:^(AMapStep *step, NSUInteger idx, BOOL *stop) {
            ...
            MAPolyline *stepPolyline = [MyBus_MANaviRoute polylineForStep:step];
            ...
            if (stepPolyline != nil)
            {
                ...
                {
                    // 填充step和step之间的空隙
                    [MyBus_MANaviRoute replenishPolylinesForPathWith:stepPolyline
                                                  lastPolyline:[MyBus_MANaviRoute polylineForStep:[path.steps objectAtIndex:idx-1]]
                                                     Polylines:polylines];
                }
            }
        }];
